...
def prep_pointcloud(input_dict,
    ...
    if not multi_gpu:
        ...
        voxels, coordinates, num_points = voxel_generator.generate_multi_gpu(
            points, max_voxels)
    example = {
        'voxels': voxels,
...
def _read_and_prep_v9(info, root_path, num_point_features, prep_func):
    ...
    # jiashan_rootpath = "/home/songhongli/calib_online_cuiwei/training/velodyne/"
...
def _read_and_prep_v9_inference(points, prep_func):
    """read data from KITTI-format infos, then call prep function.
    """
    # image_idx = info['image_idx']
    # rect = info['calib/R0_rect'].astype(np.float32)
...
def prep_pointcloud_inference(input_dict,
                    root_path,
                    voxel_generator,
                    target_assigner,
                    db_sampler=None,
