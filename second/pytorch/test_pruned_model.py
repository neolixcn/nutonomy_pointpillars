...
from second.builder_t import target_assigner_builder, voxel_builder
from second.data.preprocess import merge_second_batch
from second.protos import pipeline_pb2
from second.pytorch.builder_t import (box_coder_builder, input_reader_builder,
...
def evaluate(config_path,
    ...
    if pruned_flag:
        ...
        net_pruned = second_builder.build(model_cfg, voxel_generator, target_assigner, input_cfg.batch_size, cfg)
        net_pruned.cuda()
        torchplus.train.try_restore_latest_checkpoints(model_dir_eval, [net_pruned])
        net= net_pruned
        del(net_pruned)
    
    rpn_input = torch.ones([1, 64, 288, 288], dtype=torch.float32).cuda()
    torch.onnx.export(net.rpn, rpn_input, "rpn.onnx", verbose=True, input_names=["rpn_input_features"], output_names=["box", "cls", "dir"])

    if read_predict_pkl_path is not None:
        gt_annos = [info["annos"] for info in eval_dataset.dataset.kitti_infos]
        result_path_step = result_path / f"step_{net.get_global_step()}"
        result_path_step.mkdir(parents=True, exist_ok=True)
        print(read_predict_pkl_path + "result.pkl")
        ...
        for example in iter(eval_dataloader):
            ...
            if pickle_result:
                ...
                print(result_path_step / "result.pkl")
