...
class RPN(nn.Module):
    def __init__(self,
        ...
        if cfg is not None:
            ...
            self.block1 = Sequential(
                nn.ZeroPad2d(1),
                Conv2d(num_input_filters, block_1[0], 3, stride=layer_strides[0]),
                BatchNorm2d(block_1[0]),
                nn.ReLU(),
            )
            for idx in range(3):
                self.block1.add(Conv2d(block_1[idx], block_1[idx + 1], 3, padding=1))
                self.block1.add(BatchNorm2d(block_1[idx + 1]))
                self.block1.add(nn.ReLU())
            self.deconv1 = Sequential(
                ConvTranspose2d(
                    block_1[-2],
                    ...
                    upsample_strides[0],
                    stride=upsample_strides[0]),
                BatchNorm2d(block_1[-1]),
                nn.ReLU(),
            )

            self.block2 = Sequential(
                nn.ZeroPad2d(1),
                Conv2d(block_1[-2], block_2[0], 3, stride=layer_strides[1]),
                BatchNorm2d(block_2[0]),
                nn.ReLU(),
            )
            for idx in range(5):
                self.block2.add(Conv2d(block_2[idx], block_2[idx + 1], 3, padding=1))
                self.block2.add(BatchNorm2d(block_2[idx + 1]))
                self.block2.add(nn.ReLU())
            self.deconv2 = Sequential(
                ConvTranspose2d(
                    block_2[-2],
                    ...
                    upsample_strides[1],
                    stride=upsample_strides[1]),
                BatchNorm2d(block_2[-1]),
                nn.ReLU(),
            )

            self.block3 = Sequential(
                nn.ZeroPad2d(1),
                Conv2d(block_2[-2], block_3[0], 3, stride=layer_strides[2]),
                BatchNorm2d(block_3[0]),
                nn.ReLU(),
            )
            for idx in range(5):
                self.block3.add(Conv2d(block_3[idx], block_3[idx + 1], 3, padding=1))
                self.block3.add(BatchNorm2d(block_3[idx + 1]))
                self.block3.add(nn.ReLU())
            self.deconv3 = Sequential(
                ConvTranspose2d(
                    block_3[-2],
            ...
            if encode_background_as_zeros:
                ...
                num_cls = num_anchor_per_loc * (num_class + 1)
            summmmm = block_1[-1] + block_2[-1] + block_3[-1]
            self.conv_cls = nn.Conv2d(summmmm, num_cls, 1)
            self.conv_box = nn.Conv2d(summmmm, num_anchor_per_loc * box_code_size, 1)
            if use_direction_classifier:
                self.conv_dir_cls = nn.Conv2d(
                    summmmm, num_anchor_per_loc * 2, 1)
...
class VoxelNet(nn.Module):
    ...
    def zero_global_step(self):
        self.global_step = torch.zeros(1)
